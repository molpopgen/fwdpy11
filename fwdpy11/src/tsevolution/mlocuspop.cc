// Wright-Fisher simulation for a fwdpy11::MlocusPop with
// tree sequences.
//
...
wfMlocusPop_ts(
    ...
    const std::vector<double> &mu_selected,
    ...
    const bool suppress_edge_table_indexing, bool record_genotype_matrix)
{
    ...
    if (std::any_of(begin(mu_selected), end(mu_selected),
    ...
    std::vector<std::function<std::vector<fwdpp::uint_t>(
        fwdpp::flagged_mutation_queue &, fwdpy11::MlocusPop::mcont_t &)>>
        bound_mmodels;
    std::size_t nloci = pop.locus_boundaries.size();
    for (std::size_t i = 0; i < nloci; ++i)
        {
            auto rec
                = [i, &rng, &rmodels]() { return rmodels.regions[i](rng); };
            intralocus_recombination.emplace_back(std::move(rec));
            auto make_mutation
                = [i, &rng, &mmodels,
            ...
            const double mutrate_i = mu_selected[i];
            auto bound_mmodel_locus_i
                = [make_mutation, mutrate_i,
                   &rng](fwdpp::flagged_mutation_queue &recycling_bin,
                         std::vector<fwdpy11::Mutation> &mutations) {
                      unsigned nmuts = gsl_ran_poisson(rng.get(), mutrate_i);
                      std::vector<fwdpp::uint_t> rv;
                      for (unsigned m = 0; m < nmuts; ++m)
                          {
                              rv.push_back(
                                  make_mutation(recycling_bin, mutations));
                          }
            ...
            bound_mmodels.emplace_back(std::move(bound_mmodel_locus_i));
        }
    ...
    auto calculate_fitness
        = wrap_calculate_fitness_MlocusPop(record_genotype_matrix);
...
void
init_mlocus_evolution(py::module &m)
{
    m.def("WFMlocusPop_ts", &wfMlocusPop_ts);
}
