...
namespace fwdpy11
{
    ...
    void
    evolve_generation_ts_old(
        const rng_t& rng, poptype& pop, const fwdpp::uint_t N_next,
        const double mu, const pick_parent1_fxn& pick1,
        const pick_parent2_fxn& pick2,
        const offspring_metadata_fxn& update_offspring,
        const mutation_model& mmodel,
        fwdpp::flagged_mutation_queue& mutation_recycling_bin,
        const breakpoint_function& recmodel, const fwdpp::uint_t generation,
        std::int32_t first_parental_index, std::int32_t next_index)
    {
