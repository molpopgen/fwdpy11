...
namespace fwdpy11
{
    namespace discrete_demography
    {
        ...
        mass_migrations_and_current_sizes(
            ...
            std::vector<METADATATYPE> &metadata, DiscreteDemography &demography,
            demographic_model_state &current_demographic_state)
        {
        ...
        template <typename METADATATYPE>
        inline void
        finalize_demographic_state(const std::uint32_t generation,
                                   ...
                                   demographic_model_state &current_demographic_state)
        {
            ...
            detail::validate_parental_state(generation, current_demographic_state);
        }

        template <typename METADATATYPE>
        inline void
        update_demography_manager(const GSLrng_t &rng, const std::uint32_t generation,
                                  std::vector<METADATATYPE> &metadata,
                                  DiscreteDemography &demography,
                                  demographic_model_state &current_demographic_state)
        {
            mass_migrations_and_current_sizes(rng, generation, metadata, demography,
                                              current_demographic_state);
            finalize_demographic_state(generation, metadata, demography,
                                       current_demographic_state);
        }
