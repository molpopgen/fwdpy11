...
def run_sim(args):
    ...
    ts = run_msprime(user_args.N, user_args.rho, 0.0, user_args.model, msprime_seed)
    ...
    fwdpy11.infinite_sites(rng, pop, user_args.theta / pop.N / 4)
    an = pop.alive_nodes
    S2N, pi2N = process_sim(pop, an, check_fixations=True)
    rn = np.random.choice(an, size=user_args.nsam, replace=False)
