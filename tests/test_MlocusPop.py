...
class testMlocusPop(unittest.TestCase):
    ...
    def test_fitnesses(self):
        ...
        self.assertEqual(sum([i[0].w for i in self.pop.diploids]),float(self.pop.N))
    def test_labels(self):
        #All diploids should be labeled 0 to pop.N-1 
        self.assertEqual([i[0].label for i in self.pop.diploids],[i for i in range(self.pop.N)])
